...
class ReportController extends FOSRestController
{
  ...
  /**
   ...
   /**
     * @Rest\Get("/bycountry/{id}.{_format}", name="report_by_country", defaults={"_format":"json"})
     *
     * @SWG\Response(
     *     response=200,
     *     description="Gets statistics info based on passed ID Country parameter."
     ...
     * @SWG\Tag(name="Report")
     */
    public function byCountryAction(Request $request, $id ) {
      $serializer = $this->get('jms_serializer');
      $em = $this->getDoctrine()->getManager();
      $statistics = [];
      $message = "";
      ...
      try {
          ...
          $mbsNumbers = $em->getRepository('App:Certificate')->findOneBy(array('program' => 'MBS', 'country' => $country_id));
          $jrNumbers = $em->getRepository('App:Certificate')->findOneBy(array('program' => 'JR', 'country' => $country_id));
          $saNumbers = $em->getRepository('App:Certificate')->findOneBy(array('program' => 'SA', 'country' => $country_id));

          $eval = $this->evaluationReportByCountry($country_id);
          $show_evaluations = $this->showStatisticsCountry($eval, "MBS");

          $statistics = array(
            'MBS' => $mbsNumbers ? $mbsNumbers->getNumber() : 0 ,
            'JR'  => $jrNumbers  ? $jrNumbers->getNumber()  : 0 , 
            'SA'  => $saNumbers  ? $saNumbers->getNumber()  : 0 ,
            'EVALUATIONS' => $show_evaluations
          );
    ...
    private function evaluationReportByCountry($country){
      ...
      private function showStatisticsCountry($evaluations, $program ){
        $statistics = array();
        $studentsPre1 = 0;
        $studentsPre2 = 0;
        $studentsPre3 = 0;
